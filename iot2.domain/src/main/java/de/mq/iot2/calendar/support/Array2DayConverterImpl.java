...
class Array2DayConverterImpl implements  Converter<Pair<String[], Pair<Map<String, DayGroup>, Map<String, Cycle>>>, Day<?>> {
	...
	static final String INVALID_MONTH_DAY_MESSAGE = "Invalid MonthDay: %s";
	static final String WRONG_NUMBER_OF_COLUMNS_MESSAGE = "10 columns expected.";
	private final ConversionService conversionService;
	
	private Map<String,BiFunction<DayGroup, String[],Day<?>>> days = Map.of(GaussDayImpl.ENTITY_NAME, this::gaussDay, DayOfMonthImpl.ENTITY_NAME , this::dayOfMonthDay, DayOfWeekDayImpl.ENTITY_NAME, this::dayOfWeekDay, LocalDateDayImp.ENTITY_NAME, this::localDateDay);
	...
	private Day<?> day(final DayGroup dayGroup, final Object value, final String description, Class<?> type) {
		return description.isEmpty() ? (Day<?>) BeanUtils.instantiateClass(ReflectionUtils.findConstructor(type, dayGroup, value).orElseThrow(), dayGroup, value) :  (Day<?>) BeanUtils.instantiateClass(ReflectionUtils.findConstructor(type, dayGroup, value, description).orElseThrow(), dayGroup, value, description);
	}
	...
	private Day<?> dayOfMonthDay(final DayGroup dayGroup, final String[] columns) {
		...
		Assert.isTrue(values.length==2, String.format(INVALID_MONTH_DAY_MESSAGE, valueString));
	...
	private Day<?> localDateDay(final DayGroup dayGroup, final String[] columns) {
		final String valueString = value(columns, 1);
		final String values[] = valueString.split("[.]");
		Assert.isTrue(values.length==3, String.format(INVALID_MONTH_DAY_MESSAGE, valueString));
