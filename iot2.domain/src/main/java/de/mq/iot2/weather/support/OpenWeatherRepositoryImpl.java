...
public class OpenWeatherRepositoryImpl implements WeatherRepository {
	...
	OpenWeatherRepositoryImpl(final RestOperations restOperations,
			final @Qualifier("meteorologicalDataConverter") Converter<Map<String, Object>, MeteorologicalData> meteorologicalDataConverter,
			@Value("${iot2.openweather.city}") final String city, @Value("${iot2.openweather.country:de}") final String country,
			@Value("${iot2.openweather.key}") final String key) {
	...
	public Collection<MeteorologicalData> forecast() {
		...
		final Map<String, Object> res = restOperations.getForObject(OPEN_WEATHER_FORE_CAST_URL, Map.class,
				Map.of(PARAMETER_CITY, city, PARAMETER_COUNTRY, country, PARAMETER_KEY, key));
